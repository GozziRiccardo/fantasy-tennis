// src/pages/Admin.jsx
//
// Accessible only to the admin user (you).
// Lets you:
//   - Add Miami Open (and future tournaments)
//   - Assign players to each user's roster + set price paid
//   - Set credits remaining per user
//   - Remove a player from someone's roster
//
// To restrict access: set your own email in ADMIN_EMAIL below.
  ...
  return (
    <div className="card">
      ...
      <form onSubmit={addTournament} className="admin-form">
        ...
        <div className="field">
          <label className="label">API Tournament ID <span style={{ color: 'var(--text3)', fontWeight: 400 }}>(opzionale, aggiungibile dopo)</span></label>
...
function RosterSection() {
  ...
  async function removeFromRoster(rosterEntry) {
    ...
    if (!confirm(`Rimuovere ${name} dalla rosa? I ${price} crediti verranno restituiti.`)) return
  ...
  const filteredAtp   = search.trim().length >= 2
    ? atpPlayers.filter(p =>
        p.name.toLowerCase().includes(search.toLowerCase()) && !rosterIds.has(p.id)
      )
    : []
          ...
          <div className="roster-admin-grid">
            ...
            <div>
              <div className="admin-sub-label">
                Rosa attuale — {roster.length}/10 giocatori · {credits} crediti rimasti
              </div>
              {roster.length === 0 && (
                ...
                {roster.map(r => {
                  ...
                  return (
                    <div key={r.id} className="roster-admin-row">
                      <span className="mono" style={{ fontSize: 11, color: 'var(--text2)', width: 28 }}>
                        #{p?.ranking}
                      </span>
              ...
              <div>
                    ...
                    {filteredAtp.slice(0, 8).map(p => (
                      ...
                      >
                        ...
                        <span className="mono" style={{ fontSize: 11, color: 'var(--text2)' }}>
                          base: {computePrice(p.ranking)}cr
                        </span>
                      </button>
                    ))}
                  </div>
                ...
                {search.trim().length >= 2 && filteredAtp.length === 0 && (
                  <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: 8 }}>
                    Nessun risultato. Il giocatore potrebbe non essere ancora nel DB — aggiungilo prima dalla tabella ATP in "La mia rosa".
                  </p>
                )}
              </div>
            )}
